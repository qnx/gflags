#!/bin/sh
# The next few lines is needed if you are running the tests manually in the
# x86_64 test image:
cp ../build/nto-x86_64-o/build/bin/* . # copy the tests here
cp ../../test/flagfile.* . # copy the test input here
cp ../../test/gflags_unittest_flagfile .
export LD_LIBRARY_PATH=../build/nto-x86_64-o/build/lib/:$LD_LIBRARY_PATH
ln -s /data/gflags/qnx/build/nto-x86_64-o/build/lib/libgflags.so.2.2.2 /data/gflags/qnx/build/nto-x86_64-o/build/lib/libgflags.so.2.2
...
add_gflags_test(){
    ...
    if [ "$expected_out" != "" ]; then
        case "$output" in
            *"$expected_out"*) echo "QNXPASS: \"$expected_out\" in out"  ;;
            *)                   echo "QNXFAIL: \"$expected_out\" not in \"$output\""; overall_rc=1; return 
        esac
    fi

    if [ "$unexpected_out" != "" ]; then
        case "$output" in
            *"$unexpected_out"*) echo "QNXFAIL: \"$unexpected_out\" in \"$output\""; overall_rc=1; return  ;;
            *)                     echo "QNXPASS: \"$unexpected_out\" in out"
        esac
    fi
}
